...
class Comment < ApplicationRecord
  ...
  def users_following_thread
    ...
    users_following_thread << self.story.user if self.user.id != self.story.user.id && self.story.user_is_following
  ...
  def is_deletable_by_user?(user)
    if user && user.is_moderator?
      ...
      self.created_at >= DELETEABLE_DAYS.days.ago
    else
      false
    end
  end
  ...
  def is_editable_by_user?(user)
    return false unless user && user.id == self.user_id
      return false if self.is_moderated?


        (Time.current.to_i - (self.updated_at ? self.updated_at.to_i :
  ...
  def vote_summary_for_user(u)
    ...
    r_counts.keys.map { |k|
      ...
      o << " (#{r_users[k].join(', ')})" if u && u.is_moderator? && self.user_id != u.id
      o
    }.compact.join(", ")
  end
