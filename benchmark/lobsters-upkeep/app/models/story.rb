class Story < ApplicationRecord
  ...
  validate do
    ...
    errors.add(:title, " may not contain graphic codepoints") if self.title.match(GRAPHICS_RE)

    self.user_is_author = true if !errors.any? && self.url.blank?
  ...
  def check_not_new_domain_from_new_user
    ...
    return unless self.user && self.user.is_new? && self.domain.stories.not_deleted.count == 0
  ...
  def check_not_tracking_domain
    ...
    return unless self.domain.banned?
  ...
  def similar_stories
    ...
    @_similar_stories = @_similar_stories.where('id != ?', self.merged_story_id) if self.merged_story_id?
    @_similar_stories
  end

  def public_similar_stories(_user)
  ...
  def can_have_suggestions_from_user?(user)
    return false if !user || (user.id == self.user_id) || !user.can_offer_suggestions?
  ...
  def check_tags
    ...
    return unless self.taggings.reject { |t| t.marked_for_destruction? || t.tag.is_media? }.empty?
  ...
  def is_editable_by_user?(user)
    if user && user.is_moderator?
      ...
      if self.is_moderated?
        ...
        (Time.current.to_i - self.created_at.to_i < (60 * MAX_EDIT_MINS))
      end
    else
      false
    end
  ...
  def tags_a=(new_tag_names_a)
    self.taggings.each do |tagging|
      tagging.mark_for_destruction if !new_tag_names_a.include?(tagging.tag.tag)
    end
  ...
  def save_suggested_tags_a_for_user!(new_tag_names_a, user)
    ...
    st.each do |tagging|
      tagging.destroy if !new_tag_names_a.include?(tagging.tag.tag)
    end
    ...
    return unless final_tags.any? && (final_tags.sort != self.tags_a.sort)
  ...
  def update_merged_into_story_comments
    return unless self.merged_into_story
  ...
  def fetched_attributes_html
    ...
    begin
      ...
      if site_name.present? &&
        ...
        title = title[0, title.length - 3] if / [ \-\|\u2013] $/.match?(title)
      end
    rescue
    end
