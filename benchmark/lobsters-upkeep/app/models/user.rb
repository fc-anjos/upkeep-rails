class User < ApplicationRecord
  ...
  validates_each :username do |record, attr, value|
    record.errors.add(attr, "is not permitted") if BANNED_USERNAMES.include?(value.to_s.downcase) || value.starts_with?('tag-')
  end
  ...
  def as_json(_options = {})
    ...
    h[:github_username] = self.github_username if self.github_username.present?

    h[:twitter_username] = self.twitter_username if self.twitter_username.present?

    h[:keybase_signatures] = self.keybase_signatures if self.keybase_signatures.present?
  ...
  def create_mailing_list_token
    return unless self.mailing_list_token.blank?
      self.mailing_list_token = Utils.random_str(10)
  end

  def create_rss_token
    return unless self.rss_token.blank?
  ...
  def pushover!(params)
    return unless self.pushover_user_key.present?
