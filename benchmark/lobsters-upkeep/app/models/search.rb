class Search
  ...
  def page_count
    ...
    total = self.max_matches if total == -1 || total > self.max_matches
  ...
  def search_for_user!(user)
    ...
    when "comments"
      ...
      base = base.where(Arel.sql("MATCH(comment) AGAINST('#{qwords}' IN BOOLEAN MODE)")) if qwords.present?
      self.results = base.select(
        "comments.*, " +
    ...
    self.page = self.page_count if self.page > self.page_count
