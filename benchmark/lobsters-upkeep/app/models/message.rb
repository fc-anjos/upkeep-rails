class Message < ApplicationRecord
  ...
  validate :hat do
    next if hat.blank?
    errors.add(:hat, 'not wearable by author') if author.blank? || author.wearable_hats.exclude?(hat)
  end
  ...
  def check_for_both_deleted
    return unless self.deleted_by_author? && self.deleted_by_recipient?
      self.destroy
  end
  ...
  def deliver_email_notifications
    ...
    return unless self.recipient.pushover_messages?
