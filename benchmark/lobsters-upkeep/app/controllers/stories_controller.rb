class StoriesController < ApplicationController
  ...
  def create
    ...
    if @story.valid? && !(@story.already_posted_recently? && !@story.seen_previous) && ActiveRecord::Base.transaction { @story.save }
  ...
  def destroy
    ...
    Keystore.increment_value_for("user:#{@story.user.id}:stories_deleted") if @story.save
  ...
  def edit
    ...
    return unless @story.merged_into_story
  ...
  def new
    ...
    return unless params[:url].present?
      ...
      return unless !@story.title.present? && params[:title].present?
  ...
  def show
    ...
    respond_to do |format|
      format.html {
        ...
        @meta_tags["twitter:creator"] = "@" + @story.user.twitter_username if @story.user.twitter_username.present?
  ...
  def undelete
    ...
    Keystore.increment_value_for("user:#{@story.user.id}:stories_deleted", -1) if @story.save
  ...
  def update
    ...
    return redirect_to @story.comments_path if @story.save
  ...
  def upvote
    ...
    return render plain: "story has been merged", status: 400 if story.merged_into_story
  ...
  def flag
    ...
    return render plain: "invalid reason", status: 400 if !Vote::STORY_REASONS[params[:reason]]

    return render plain: "not permitted to flag", status: 400 if !@user.can_flag?(story)
...
private
  ...
  def verify_user_can_submit_stories
    return unless !@user.can_submit_stories?
