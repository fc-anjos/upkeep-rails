class AvatarsController < ApplicationController
  ...
  def show
    ...
    raise ActionController::RoutingError.new("invalid size") if !ALLOWED_SIZES.include?(size)

    raise ActionController::RoutingError.new("invalid user name") if !username.match(User::VALID_USERNAME)
    ...
    Dir.mkdir(CACHE_DIR) if !Dir.exist?(CACHE_DIR)
