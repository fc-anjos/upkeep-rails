class SettingsController < ApplicationController
  ...
  def twofa_auth
    if @user.authenticate(params[:user][:password].to_s)
      ...
      return redirect_to twofa_enroll_url unless @user.has_2fa?
        @user.disable_2fa!
  ...
  def twofa_enroll
    ...
    session[:totp_secret] = ROTP::Base32.random if !session[:totp_secret]
  ...
  def pushover_callback
    ...
    raise "rand param #{params[:rand].inspect} != #{session[:pushover_rand].inspect}" if params[:rand].to_s != session[:pushover_rand].to_s
  ...
  def github_callback
    ...
    return github_disconnect unless tok.present? && username.present?
  ...
  def twitter_callback
    ...
    return twitter_disconnect unless tok.present? && username.present?
