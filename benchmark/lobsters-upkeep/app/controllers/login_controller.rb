...
class LoginController < ApplicationController
  ...
  def login
    ...
    begin
      raise LoginFailedError if !user

      raise LoginWipedError if user.is_wiped?

      raise LoginFailedError if !user.authenticate(params[:password].to_s)

      raise LoginBannedError if user.is_banned?

      raise LoginDeletedError if !user.is_active?
...
private
  ...
  def find_twofa_user
    return unless session[:twofa_u].present?
