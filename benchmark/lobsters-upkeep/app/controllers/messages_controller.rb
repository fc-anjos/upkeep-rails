class MessagesController < ApplicationController
  ...
  def create
    ...
    if @new_message.save
      ModNote.create_from_message(@new_message, @user) if @user.is_moderator? && @new_message.mod_note
  ...
  def destroy
    ...
    return redirect_to "/messages/sent" if @message.author_user_id == @user.id
...
private
  ...
  def find_message
    return true if (@message = Message.where(short_id: params[:message_id] || params[:id]).first) && (@message.author_user_id == @user.id || @message.recipient_user_id == @user.id)
