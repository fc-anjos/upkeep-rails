class CommentsController < ApplicationController
  ...
  before_action :require_logged_in_user_or_400,
                only: [ :create, :upvote, :flag, :unvote ]
  ...
  def create
    ...
    comment.hat_id = params[:hat_id] if params[:hat_id] && @user.wearable_hats.where(id: params[:hat_id])
  ...
  def flag
    ...
    return render plain: "invalid reason", status: 400 if !Vote::COMMENT_REASONS[params[:reason]]

    return render plain: "not permitted to flag", status: 400 if !@user.can_flag?(comment)
  ...
  def upvoted
    ...
    respond_to do |format|
      format.html { render action: :index }
      format.rss {
        @title = "Upvoted comments feed for #{@user.username}" if @user && params[:token].present?
