...
class Sponge
  ...
  def fetch(url, method = :get, fields = nil, raw_post_data = nil, headers = {}, limit = 10)
    ...
    begin
      ...
      raise DNSError.new("couldn't resolve #{uri.host} (DNS timeout)") if retried
    ...
    raise DNSError.new("couldn't resolve #{uri.host}") if !ip
    ...
    host.set_debug_output $stdout if self.debug
