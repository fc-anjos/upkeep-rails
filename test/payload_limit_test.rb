...
class PayloadLimitTest < ActionDispatch::IntegrationTest
  ...
  def promote_shared_board
    ...
    [s, a_sess.response.headers["X-RefreshSync-Stream"]]
  end

  def test_oversized_payload_degrades_to_refresh_and_warns
    s, cohort_stream = promote_shared_board
    RefreshSync.payload_limit = 64 # far below any rendered partial

    events = []
    callback = ->(*, payload) { events << payload }
    ActiveSupport::Notifications.subscribed(callback, "payload_limit_degrade.refresh_sync") do
    ...
    assert_operator RefreshSync.stats[:payload_limit_degrades], :>=, 1
  end
  ...
  def test_small_payload_broadcasts_normally_under_a_generous_limit
    ...
    assert_equal 0, RefreshSync.stats[:payload_limit_degrades]
  end
end
